...
export class SummarizerAgent {
  ...
  async summarize(rawText) {
    this.log(`[Summarizer Agent] Đang phân tích và tóm tắt tài liệu (${rawText.length} ký tự)...`);
...
${rawText.slice(0, 15000)}`;
        ...
        this.log(`[Summarizer Agent] Đã tóm tắt thành công bằng Gemini AI.`);
        return text;
      } catch (error) {
        this.log(`[Summarizer Agent] Lỗi khi gọi Gemini API: ${error.message}. Chuyển sang thuật toán dự phòng.`);
      }
    }
...
export class CopywriterAgent {
  ...
  async writeContent(summary, templateType = 'educational') {
    this.log(`[Copywriter Agent] Đang lên kịch bản video và bài viết theo mẫu: ${templateType}...`);
...
Kịch bản tối thiểu 4 cảnh.`;
        ...
        const response = await model.generateContent(prompt);
        const text = response.response.text();
        this.log(`[Copywriter Agent] Đã viết kịch bản thành công bằng Gemini AI.`);
        return text;
      } catch (error) {
        this.log(`[Copywriter Agent] Lỗi khi viết kịch bản bằng AI: ${error.message}. Chuyển sang mẫu dự phòng.`);
      }
    }
  ...
  fallbackWriteContent(summary, templateType) {
    // Generate script template based on templateType
...
export class VideoDirectorAgent {
  ...
  parseStoryboard(writerOutput) {
    ...
    if (scenes.length === 0) {
      ...
      // Mock storyboard standard format fallback
      return [
